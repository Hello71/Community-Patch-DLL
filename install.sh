...
copy() {
    rsync -rlptcW --delete --inplace "$@"
}

printf '%s\n' "Installing $instype to Civ 5 at \"$civ5dir\" and data at \"$datadir\"..." >&2
...
cat >&2 << EOF
Installation complete! In order to allow loading DLL mods, replace your start command with:

    LD_PRELOAD=./libCiv5XP_hook.so %command%

Otherwise, Civilization V will crash when loading DLL mods.
EOF
