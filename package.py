...
def write_tar(fileobj: BinaryIO, files: Iterator[str], args) -> None:
    with tarfile.open(fileobj=fileobj, mode='w|') as tar:
        ...
        tar_add(args.build_dir / 'libCiv5XP_hook-stripped.so', 'libCiv5XP_hook.so')
        tar_add(args.build_dir / 'libCvGameCoreDLL_Expansion2-stripped.so', '(1) community patch/libcvgamecoredll_expansion2.so')
