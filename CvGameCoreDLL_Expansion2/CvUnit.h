...
public:
	...
	enum
	{
		...
		MOVEFLAG_PRETEND_CANALS					= 0x8000000, //pretend ships can move one tile inland to see if a canal would make sense
	    MOVEFLAG_IGNORE_STACKING_NEUTRAL		= 0x10000000, // stacking rules (with neutral units) don't apply (on turn end plots)
		MOVEFLAG_CONTINUE_TO_CLOSEST_PLOT		= 0x20000000, //if the target plot is occupied go to the closest available plot instead
		MOVEFLAG_VISIBLE_ONLY				    = 0x40000000, //workers typically should not go exploring
