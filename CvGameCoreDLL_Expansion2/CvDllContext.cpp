...
bool CvDllGameContext::RandomNumberGeneratorSyncCheck(PlayerTypes ePlayer, ICvRandom1* pRandom, bool bIsHost)
{
	...
	if(localSimRandomNumberGenerator != *pkRandom)
	{
		...
		std::ostringstream rngLogMessage;
		rngLogMessage << "Game Random Number Generators are out of sync : "
		              << "local.seed=" << localSimRandomNumberGenerator.getSeed()
		              << ", remote.seed=" << pkRandom->getSeed()
		              << "\n\tlocal.callCount=" << localSimRandomNumberGenerator.getCallCount()
		              << ", remote.callCount=" << pkRandom->getCallCount()
		              << "\n\tlocal.resetCount=" << localSimRandomNumberGenerator.getResetCount()
		              << ", remote.resetCount=" << pkRandom->getResetCount()
		              << "\n";

		if(logFile)
			logFile->DebugMsg(rngLogMessage.str().c_str());
